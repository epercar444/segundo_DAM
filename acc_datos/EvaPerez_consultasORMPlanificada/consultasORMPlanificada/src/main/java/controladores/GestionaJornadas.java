...
public class GestionaJornadas {  
	...
	public static void main(String[] args) {
		...
		//Consulta participantes ordenados por apellidos y nombre
		List<Object[]> participantesXapellidos = participanteDao.getParticipantesXOrder();
		for (Object[] p : participantesXapellidos) {
			logger.info(Arrays.toString(p));
		}
		
		//Consulta eventos filtrado por duración
		List<Object[]> eventosXduracion = eventoDao.getEventosXDuracion();
		for (Object[] e : eventosXduracion) {
			logger.info(Arrays.toString(e));
		}
		
		//Consulta que devuelve los eventos meidnate filtros, saca la lista de participantes pero no el número de participantes
		List<Object[]> eventosXfiltros = eventoDao.getEventosXFiltros();
		for (Object[] e : eventosXfiltros) {
			logger.info(Arrays.toString(e));
		}
		
		//Devuelve el resultado de pasarle por parámetro una ubicación
		List<Object[]> eventosParametrizados = eventoDao.getEventosParametrizados(u1);
		for (Object[] e : eventosParametrizados) {
			logger.info(Arrays.toString(e));
		}
